import { SecurityKey } from "./typings/terminal/securityKey";
...
class Config {
  ...
  public constructor(options: ConfigConstructor = {}) {
    ...
    if (options.legalEntityManagementEndpoint) {
      this.legalEntityManagementEndpoint =
        options.legalEntityManagementEndpoint;
    }
    if (options.transfersEndpoint) {
      this.transfersEndpoint = options.transfersEndpoint;
    }
    if (options.dataProtectionEndpoint) {
  ...
  public get checkoutEndpoint(): string | undefined {
    if (!this._checkoutEndpoint) {
      const message =
        'Please provide your unique live url prefix on the setEnvironment() call on the Client or provide checkoutEndpoint in your config object.';
