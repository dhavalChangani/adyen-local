...
class NexoCrypto {
  public static encrypt(
    ...
    const derivedKey: NexoDerivedKey =
      NexoDerivedKeyGenerator.deriveKeyMaterial(securityKey.Passphrase);
    const saleToPoiMessageByteArray = Buffer.from(
      saleToPoiMessageJson,
      'utf-8',
    );
    const ivNonce = NexoCrypto.generateRandomIvNonce();
    const encryptedSaleToPoiMessage = NexoCrypto.crypt(
      saleToPoiMessageByteArray,
      derivedKey,
      ivNonce,
      Modes.ENCRYPT,
    );
    const encryptedSaleToPoiMessageHmac = NexoCrypto.hmac(
      saleToPoiMessageByteArray,
      derivedKey,
    );
  ...
  public decrypt(
    ...
    const encryptedSaleToPoiMessageByteArray = Buffer.from(
      saleToPoiSecureMessage.NexoBlob,
      'base64',
    );
    const derivedKey = NexoDerivedKeyGenerator.deriveKeyMaterial(
      securityKey.Passphrase,
    );
    const ivNonce = Buffer.from(
      saleToPoiSecureMessage.SecurityTrailer.Nonce,
      'base64',
    );
    const decryptedSaleToPoiMessageByteArray = NexoCrypto.crypt(
    ...
    const receivedHmac = Buffer.from(
      saleToPoiSecureMessage.SecurityTrailer.Hmac,
      'base64',
    );
    this.validateHmac(
      receivedHmac,
      decryptedSaleToPoiMessageByteArray,
      derivedKey,
    );
