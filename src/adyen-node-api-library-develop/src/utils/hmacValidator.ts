...
class HmacValidator {
  ...
  public calculateHmac(
    data: string | NotificationRequestItem,
    key: string,
  ): string {
    const dataString =
      typeof data !== 'string' ? this.getDataToSign(data) : data;
    const rawKey = Buffer.from(key, 'hex');
    return createHmac(HmacValidator.HMAC_SHA256_ALGORITHM, rawKey)
  ...
  public validateHMAC(
    ...
    if (notificationRequestItem.additionalData?.[ApiConstants.HMAC_SIGNATURE]) {
      ...
      const merchantSign =
        notificationRequestItem.additionalData?.[ApiConstants.HMAC_SIGNATURE];
  ...
  private isNotificationRequestItem(
    item: DataToSign,
  ): item is NotificationRequestItem {
